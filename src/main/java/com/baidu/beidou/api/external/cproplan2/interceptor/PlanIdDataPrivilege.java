package com.baidu.beidou.api.external.cproplan2.interceptor;
...
public class PlanIdDataPrivilege extends AbstractDataPrivilege {
		...
		if (ognlExp != null) {
			...
			if (!planIds.isEmpty()) {
				...
				for (int index = 0; index < planIds.size(); index++) {
					List<Integer> userIds = planMgr.findUserIdByPlanIds(planIds.subList(index, index + 1));
					if (userIds == null || userIds.isEmpty()
							|| userIds.get(0) != user.getDataUser()) {
