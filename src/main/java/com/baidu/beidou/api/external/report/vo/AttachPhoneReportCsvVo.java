...
/**
 * ClassName: AttachPhoneReportCsvVo <br>
 * Function: 附加信息电话 csv vo
 *
 * @author wangxujin
 */
public class AttachPhoneReportCsvVo extends AbstractApiReportCsvVo<AttachPhoneStatViewItem> {
    // id only
    protected String[] idOnlyHeader = ReportWebConstants.REPORT_HEADER_IDONLY
            .get(ReportWebConstants.REPORT_TYPE.ATTACH_PHONE);

    // 非id only
    protected String[] notIdOnlyHeader = ReportWebConstants.REPORT_HEADER_NOT_IDONLY
            .get(ReportWebConstants.REPORT_TYPE.ATTACH_PHONE);

    public AttachPhoneReportCsvVo(boolean idOnly, int performanceData) {
        this.idOnly = idOnly;
    ...
    public List<String[]> getCsvReportDetail() {
        ...
        for (AttachPhoneStatViewItem item : details) {
            if (idOnly) {
                ...
                detailStringArray[i++] = String.valueOf(item.getPhoneId());
                ...
                detailStringArray[i++] = String.valueOf(item.getPhoneId());
                detailStringArray[i++] = item.getPhone();
                appendPerformanceData(detailStringArray, i, item);
            }
