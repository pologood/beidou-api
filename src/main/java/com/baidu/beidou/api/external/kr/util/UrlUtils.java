...
public class UrlUtils {
	...
	/** url前缀判断 */
	private static final Pattern URL_PREFIX_PATTERN = Pattern.compile("^(http|https|www)");
	
	private static final Pattern MAIN_DOMAIN = Pattern.compile("^(([\\w-]*\\w)\\.)+[\\w-]+$");
	...
	/**
	 * 判断输入词是否为URL、关键词或两者皆是
	 * @param urlAddress
	 * @return
	 */
	public static QUERYTYPE getKrQueryType(String urlAddress){
		boolean isUrl = isUrl(urlAddress);
		if(!isUrl){
			return QUERYTYPE.JUSTWORD;
		} else {
			if(urlAddress.trim().length() > BizConf.TOOLS_KRGW_WORDLEN){
				return QUERYTYPE.JUSTURL;
			}
			Matcher matcher = URL_PREFIX_PATTERN.matcher(urlAddress.trim().toLowerCase());
			Matcher matcherMainDomain = MAIN_DOMAIN.matcher(urlAddress.trim().toLowerCase());
			if(matcher.find()){
				return QUERYTYPE.JUSTURL; 
			}else if(urlAddress.contains("/") || urlAddress.contains(":")) {
				return QUERYTYPE.JUSTURL; 
			}else if(matcherMainDomain.find()) {
				return QUERYTYPE.URLANDWORD;
			} else {
				return QUERYTYPE.JUSTURL; 
			}
		}
	 ...
	 * 
	* @ClassName: QUERYTYPE
	* @Description: KR被动推荐，查询词类型字典
	* @author baixiaojie@baidu.com
	* @date 2011-1-5 上午11:55:26
	*
	 */
...
//		try {
//			SimpleConfig.processConfig("earth_conf_url_domain.xml");
...
//			
//            FileWriter targetUrlWriter = new FileWriter("targetUrl.txt");
//            FileWriter targetWordWriter = new FileWriter("targetWord.txt");
//            FileWriter targetWordAndUrlWriter = new FileWriter("targetWordAndUrl.txt");
//            
//            String str = null;
//            while((str = br.readLine()) != null) {
//            	QUERYTYPE queryType = getKrQueryType(str);
//            	switch (queryType) {
//				case JUSTURL:
//					targetUrlWriter.append(str + "\n");
//					break;
//				case JUSTWORD:
//					targetWordWriter.append(str + "\n");
//					break;
//				case URLANDWORD:
//					targetWordAndUrlWriter.append(str + "\n");
//					break;
//				default:
//					break;
//				}
...
//            br.close();
//            reader.close();
//            targetUrlWriter.flush();
//            targetWordAndUrlWriter.flush();
//            targetWordWriter.flush();
//            targetUrlWriter.close();
//            targetWordAndUrlWriter.close();
//            targetWordWriter.close();
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
//		
//		String ipUrl = "asp.net?#快快快";
//		QUERYTYPE queryType = getKrQueryType(ipUrl);
//		System.out.println(queryType);
//		System.out.println(isUrl(ipUrl));
//	}
}
