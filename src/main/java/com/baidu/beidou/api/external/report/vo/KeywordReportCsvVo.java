...
/**
 * 
 * ClassName: KeywordReportCsvVo <br>
 * Function: 用于写入文件报告的关键词VO
 *
 * @author zhangxu
 * @version 2.0.0
 * @since cpweb357
 * @date Jan 5, 2012
 */
public class KeywordReportCsvVo extends AbstractApiReportCsvVo<KeywordStatViewItem> {
    ...
    protected String[] idOnlyHeader = ReportWebConstants.REPORT_HEADER_IDONLY
            .get(ReportWebConstants.REPORT_TYPE.KEYWORD_NORMAL_COMPATIBLE);

    // 非id only
    protected String[] notIdOnlyHeader = ReportWebConstants.REPORT_HEADER_NOT_IDONLY
            .get(ReportWebConstants.REPORT_TYPE.KEYWORD_NORMAL_COMPATIBLE);

    public KeywordReportCsvVo(boolean idOnly, int performanceData) {
        this.idOnly = idOnly;
