...
/**
 * 
 * ClassName: UnitReportCsvVo <br>
 * Function: 用于写入文件报告的创意VO
 *
 * @author zhangxu
 * @version 2.0.0
 * @since cpweb357
 * @date Jan 5, 2012
 */
public class UnitReportCsvVo extends AbstractApiReportCsvVo<UnitStatViewItem> {

    private static final long serialVersionUID = -1035255603639830367L;

    // id only
    protected String[] idOnlyHeader = ReportWebConstants.REPORT_HEADER_IDONLY.get(ReportWebConstants.REPORT_TYPE.UNIT);

    // 非id only
    protected String[] notIdOnlyHeader = ReportWebConstants.REPORT_HEADER_NOT_IDONLY
            .get(ReportWebConstants.REPORT_TYPE.UNIT);

    public UnitReportCsvVo(boolean idOnly, int performanceData) {
        this.idOnly = idOnly;
    ...
    public List<String[]> getCsvReportDetail() {
        ...
        for (UnitStatViewItem item : details) {
            if (idOnly) {
                ...
                detailStringArray[i++] = String.valueOf(item.getUnitid());
                ...
                detailStringArray[i++] = String.valueOf(item.getUnitid());
                detailStringArray[i++] = item.getTitle();
                detailStringArray[i++] = String.valueOf(item.getWuliaotype());
                appendPerformanceData(detailStringArray, i, item);
            }
