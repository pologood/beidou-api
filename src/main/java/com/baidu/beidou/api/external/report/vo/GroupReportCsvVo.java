...
/**
 * 
 * ClassName: GroupReportCsvVo <br>
 * Function: 用于写入文件报告的推广组VO
 *
 * @author zhangxu
 * @version 2.0.0
 * @since cpweb357
 * @date Jan 5, 2012
 */
public class GroupReportCsvVo extends AbstractApiReportCsvVo<GroupStatViewItem> {

    private static final long serialVersionUID = -1035255603639830367L;

    // id only
    protected String[] idOnlyHeader = ReportWebConstants.REPORT_HEADER_IDONLY.get(ReportWebConstants.REPORT_TYPE.GROUP);

    // 非id only
    protected String[] notIdOnlyHeader = ReportWebConstants.REPORT_HEADER_NOT_IDONLY
            .get(ReportWebConstants.REPORT_TYPE.GROUP);

    public GroupReportCsvVo(boolean idOnly, int performanceData) {
        this.idOnly = idOnly;
    ...
    public List<String[]> getCsvReportDetail() {
        ...
        for (GroupStatViewItem item : details) {
            if (idOnly) {
