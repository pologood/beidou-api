...
/**
 * 
 * ClassName: RegionReportCsvVo <br>
 * Function: 从doris查询出来的性别数据VO
 *
 * @author zhangxu
 * @date Sep 25, 2012
 */
public class RegionReportCsvVo extends AbstractApiReportCsvVo<RegionStatViewItem> {
    ...
    protected String[] idOnlyHeader = ReportWebConstants.REPORT_HEADER_IDONLY
            .get(ReportWebConstants.REPORT_TYPE.REGION);

    // 非id only
    protected String[] notIdOnlyHeader = ReportWebConstants.REPORT_HEADER_NOT_IDONLY
            .get(ReportWebConstants.REPORT_TYPE.REGION);

    public RegionReportCsvVo(boolean idOnly, int performanceData) {
        this.idOnly = idOnly;
    ...
    public List<String[]> getCsvReportDetail() {
        ...
        for (RegionStatViewItem item : details) {
            if (idOnly) {
                ...
                detailStringArray[i++] = String.valueOf(item.getRegionId());
                detailStringArray[i++] = String.valueOf(item.getType());
                detailStringArray[i++] = String.valueOf(item.getFirstRegionId());
                ...
                detailStringArray[i++] = String.valueOf(item.getRegionId());
                detailStringArray[i++] = item.getRegionName();
                detailStringArray[i++] = String.valueOf(item.getType());
                detailStringArray[i++] = String.valueOf(item.getFirstRegionId());
                appendPerformanceData(detailStringArray, i, item);
            }
