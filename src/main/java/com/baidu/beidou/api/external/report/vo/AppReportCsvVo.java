...
/**
 * 
 * ClassName: AppReportCsvVo <br>
 * Function: 用于写入文件报告的有展现应用推广计划VO
 *
 * @author caichao
 * @version 2.0.0
 * @since cpweb357
 * @date 11 21, 2013
 */
public class AppReportCsvVo extends AbstractApiReportCsvVo<AppStatViewItem> {

    private static final long serialVersionUID = -1035255603639830367L;

    // id only
    protected String[] idOnlyHeader = ReportWebConstants.REPORT_HEADER_IDONLY.get(ReportWebConstants.REPORT_TYPE.APP);

    // 非id only
    protected String[] notIdOnlyHeader = ReportWebConstants.REPORT_HEADER_NOT_IDONLY
            .get(ReportWebConstants.REPORT_TYPE.APP);

    public AppReportCsvVo(boolean idOnly, int performanceData) {
        this.idOnly = idOnly;
    ...
    public List<String[]> getCsvReportDetail() {
        ...
        for (AppStatViewItem item : details) {
            if (idOnly) {
                ...
                detailStringArray[i++] = String.valueOf(item.getAppId());
                ...
                detailStringArray[i++] = String.valueOf(item.getAppId());
                detailStringArray[i++] = String.valueOf(item.getAppName());
                appendPerformanceData(detailStringArray, i, item);
            }
