 ...
 * @date 2012-1-10
 */
public class CampaignServiceImpl2 implements CampaignService2 {
	...
	public ApiResult<CampaignType> addCampaign(DataPrivilege user,
		...
		int userWebSiteTag = userInfoMgr.getUserWebsiteTagFromDrm(user.getDataUser());
		...
		for (int index = 0; index < planNum; index++) {
            ...
            if (campaignType.getType() == null) {
                if (UserConstant.USER_WEB_SITE_TAG_WIRELESS == userWebSiteTag) {
                    campaignType.setType(CproPlanConstant.PROMOTIONTYPE_WIRELESS);
                } else {
                    campaignType.setType(CproPlanConstant.PROMOTIONTYPE_ALL);
                }
            } else {
                // 用户注册网站类型设置为仅移动，则只能创建仅移动的推广组
                if (UserConstant.USER_WEB_SITE_TAG_WIRELESS == userWebSiteTag
                        && CproPlanConstant.PROMOTIONTYPE_WIRELESS != campaignType.getType()) {
                    ApiErrorPosition apiPosition = new ApiErrorPosition(PositionConstant.PARAM);
			...
			if(campaignType.getWirelessBidRatio() == null){
				campaignType.setWirelessBidRatio(CproPlanConstant.WIRELESSBIDRATIO_DEFAULT);
			}
	...
	public ApiResult<CampaignType> getCampaignByCampaignId(DataPrivilege user,
		...
		for (CproPlan cproPlan : cproPlanList) {
			...
			plan.setDevice(new Long(DeviceOsUtil.getDeviceValue(deviceOperaSystem)).intValue());
			plan.setOs(CollectionsUtil.tranformLongListToLongArray(DeviceOsUtil.getOSValue(deviceOperaSystem)));
			
			//add by wangxiongjie for atright
			plan.setReserved1(cproPlan.getPlanType());
	...
	public ApiResult<CampaignType> getCampaign(DataPrivilege user,
		...
		//临时过滤atleft计划 add by caichao
		Iterator<CproPlan> iterator = cproPlanList.iterator();
		while(iterator.hasNext()){
			CproPlan plan = iterator.next();
