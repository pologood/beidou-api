...
import com.baidu.api.sem.nms.v2.GetSiteConfigRequest;
import com.baidu.api.sem.nms.v2.GetSiteConfigResponse;
import com.baidu.api.sem.nms.v2.GroupConfigService;

/**
 * ClassName: GetSiteConfigExample
 * Function: TODO ADD FUNCTION
 *
 * @author Baidu API Team
 * @date 2012-2-13
 */
public class GetSiteConfigExample {
	private GroupConfigService service;

	public GetSiteConfigExample() {
	...
	public GetSiteConfigResponse getSiteConfig(long[] groupIds) {
		// Prepare your parameters.
		GetSiteConfigRequest parameters = new GetSiteConfigRequest();
		for (long groupId : groupIds) {
			parameters.getGroupIds().add(groupId);
		}
		// Invoke the method.
		GetSiteConfigResponse ret = service.getSiteConfig(parameters);
		...
		if (rheader.getStatus() == 0) {
			System.out.println("result\n" + ObjToStringUtil.objToString(ret.getSiteConfigs()));
			return ret;
		} else {
	...
	public static void main(String[] args) throws Throwable {
		GetSiteConfigExample example = new GetSiteConfigExample();
		long[] groupIds = new long[] { 2166146, 2166147, 2166149 };
		example.getSiteConfig(groupIds);
	}
}
