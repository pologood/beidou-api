...
import com.baidu.api.sem.nms.v2.GetRegionConfigRequest;
import com.baidu.api.sem.nms.v2.GetRegionConfigResponse;
import com.baidu.api.sem.nms.v2.GroupConfigService;

/**
 * ClassName: GetRegionConfigExample
 * Function: TODO ADD FUNCTION
 *
 * @author Baidu API Team
 * @date 2012-2-13
 */
public class GetRegionConfigExample {
	private GroupConfigService service;

	public GetRegionConfigExample() {
	...
	public GetRegionConfigResponse getRegionConfig(long[] groupIds) {
		// Prepare your parameters.
		GetRegionConfigRequest parameters = new GetRegionConfigRequest();
		for (long groupId : groupIds) {
			parameters.getGroupIds().add(groupId);
		}
		// Invoke the method.
		GetRegionConfigResponse ret = service.getRegionConfig(parameters);
		...
		if (rheader.getStatus() == 0) {
			System.out.println("result\n" + ObjToStringUtil.objToString(ret.getRegionConfigs()));
			return ret;
		} else {
	...
	public static void main(String[] args) throws Throwable {
		GetRegionConfigExample example = new GetRegionConfigExample();
		long[] groupIds = new long[] { 2166146, 2166147, 2166149 };
		example.getRegionConfig(groupIds);
	}
}
